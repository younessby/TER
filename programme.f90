program programme
   ...
   ! print*,"c'est bon"
    ...
    allocate(poin_tri(3*nb_point))
    nb_point_aj=0
        do i=1,nb_tri1
            if(hasObtuseAngle(triangles_tri(i)%p1,triangles_tri(i)%p2,triangles_tri(i)%p3) .eqv. .false.) then
    ...
    deallocate(poin_tri,poin_tri1,lespoints)
    allocate(lespoints(nb_point))
    lespoints=point_delaunay

    deallocate(point_delaunay,triangles_tri,lestriangles,triangles_tri_super)
...
!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Voronoi %%%%%%%%%
    ...
    allocate(aretes_delaunay(nb_aretes),edge_bord1(nb_aretes),point_bord1(nb_aretes))
        !trouver les arrtes et le spoints au bord
    compteur_edge = 0
    compteur_point=0
    do i=1,nb_aretes
...
!    !nb_cel=nb_cel+nb_point-3
   ...
   write(10, '(A,I0,A)') "POINTS ", nb_tri1, " float"
   do i = 1, nb_tri1
